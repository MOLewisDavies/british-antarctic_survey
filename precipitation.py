...
def get_stats(index, site, shape_file):
    ...
    for file in precip_files:
        ...
        time_grid, lat_grid, lon_grid = np.meshgrid(time, lat, lon, 
                                                    indexing='ij')
    ...
    big_df.to_csv(f'/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/csv_ouputs/precip_{site}_stats.csv')
...
def make_area_heatmap(area_df):
    ...
    ## create total column 
    area_df['total'] = ((area_df['jan'] + area_df['feb'] + area_df['oct'] + area_df['nov'] + area_df['dec'])/4650)*100
    ...
    for month in months:
        ...
        # Set extent of plot
        ax.set_extent([min(lons) - 0.5, max(lons) + 0.5, 
        ...
        area_df['norm'] = (area_df[f'{month}']-area_df[f'{month}'].min())/(area_df[f'{month}'].max() - area_df[f'{month}'].min())
        ...
        patches = [plt.Rectangle((lon-0.125, lat-0.125), 0.25, 0.25, alpha=1, facecolor=cmap(precip)) for lon,lat, precip in zip(lons,lats,precip_norm)]
        ...
        ## add colourbar
        plt.colorbar(sm, ax=ax, label='No. of Flyable Days', ticks =[precip_data.min(),precip_data.median(),  precip_data.max()])
        
        ## save figure
        fig.savefig(f'/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/{month}_all_sites_precip_heatmap.png')
...
def make_site_heatmap(flyable_df, d_type, site):
    ...
    ## create total column 
    flyable_df['total'] = (flyable_df['jan'] + flyable_df['feb'] + flyable_df['oct'] + flyable_df['nov'] + flyable_df['dec'])
    ...
    for month in months:
        ...
        ## create normalised data column
        flyable_df['norm'] = (flyable_df[f'{month}']-flyable_df[f'{month}'].min())/(flyable_df[f'{month}'].max() - flyable_df[f'{month}'].min())
        ...
        ## save figure
        fig.savefig(f'/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/{month}_{site}_precip_heatmap.png')
...
def make_box_whisker_plots(grid_points, d_type):
    ...
    ## set title
    ax.set_title(f'Flyable Days Per Month - {grid_points} grid point(s) - {d_type}', fontsize=18,
                    fontweight='bold')
    
    ## save figure         
    fig.savefig(f'/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/monhtly_precip_whisker_plot_{grid_points}_{d_type}.png')
...
def make_box_whisker_east_west(AREA, d_type):
    ...
    ## set title    
    ax.set_title(f'Flyable Days Per Month - {AREA}ERN grid point(s) - {d_type}', fontsize=18,
                    fontweight='bold')
    
    ## save figure
    fig.savefig(f'/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/monhtly_precip_whisker_plot_{AREA}_{d_type}.png')
