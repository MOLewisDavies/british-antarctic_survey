...
def make_heatmap_plots():
    ...
    for site in SITES:
        ...
        ## load dataframe
        data_df = pd.read_csv(f"csv_ouputs/visibility_{site}_stats.csv")
...
def make_area_heatmap(area_df):
    ...
    patches = [
        plt.Rectangle(
            (lon - 0.125, lat - 0.125), 0.25, 0.25, alpha=1, facecolor=cmap(vis)
        )
        for lon, lat, vis in zip(lons, lats, vis_norm)
    ]
    patch_collection = PatchCollection(patches, match_original=True)

    ## create scalar mappable for colourbar
    norm = plt.Normalize(vis_data.min(), vis_data.max())
    ...
    plt.colorbar(
        ...
        ticks=[vis_data.min(), vis_data.median(), vis_data.max()],
    )

    ## save figure
    fig.savefig(
        f"/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/total_all_sites_visibility_heatmap.png"
    )
...
def make_site_heatmap(flyable_df, d_type, site):
    ...
    for month in months:
        ...
        ## save figure
        fig.savefig(
            f"/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/{month}_{site}_visibility_heatmap.png"
        )
...
def get_stats(index, site, shape_file):
    ...
    for temp_file, lwc_file, iwc_file in zip(TEMP_FILES, LWC_FILES, IWC_FILES):
        ...
        lwc_cube = iris.load(lwc_file)[0]
        iwc_cube = iris.load(iwc_file)[0]
    ...
    big_df.to_csv(
        f"/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/csv_ouputs/visibility_{site}_stats.csv"
    )
...
def count_flyable_days(data_df, site):
    ...
    flyable_file = Path(f"csv_ouputs/visibility_{site}_fly_days.txt")
    ...
    ## otherwise
        ...
        with open(f"csv_ouputs/visibility_{site}_fly_days.txt", "wb") as file:
...
def is_day_flyable(day_df, days):
    ...
    for index, row in day_df.iterrows():
        ...
        ## otherwise
            ...
            vis_1 = row["Visibility"]
            vis_2 = day_df.loc[day_df["Date and Time"] == consec_date_str, "Visibility"]

            ## check if both precip values are below threshold
            if (vis_1 > 5000) and (vis_2.item() > 5000):
