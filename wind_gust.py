    ...
    for shape_file in SHAPES:
        ...
        for index, site in enumerate(SITES):
            ...
            if csv_file.is_file():
                ...
                print(f"wind_gust_{site}_stats.csv exists")
...
def make_heatmap_plots():
    ...
    for site in SITES:
        ...
        ## load dataframe
        data_df = pd.read_csv(f"csv_ouputs/wind_gust_{site}_stats.csv")
...
def make_area_heatmap(area_df):
    ...
    patches = [
        plt.Rectangle(
            (lon - 0.125, lat - 0.125), 0.25, 0.25, alpha=1, facecolor=cmap(gust)
        )
        for lon, lat, gust in zip(lons, lats, gust_norm)
    ]
    patch_collection = PatchCollection(patches, match_original=True)

    ## create scalar mappable for colourbar
    norm = plt.Normalize(gust_data.min(), gust_data.max())
    ...
    plt.colorbar(
        ...
        ticks=[gust_data.min(), gust_data.median(), gust_data.max()],
    )

    ## save figure
    fig.savefig(
        f"/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/total_all_sites_wind_gust_heatmap.png"
    )
...
def make_site_heatmap(flyable_df, d_type, site):
    ...
    for month in months:
        ...
        ## save figure
        fig.savefig(
            f"/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/plots/{month}_{site}_wind_gust_heatmap.png"
        )
...
def get_stats(index, site, shape_file):
    ...
    big_df.to_csv(
        f"/home/users/lewis.davies/british_antarctic_survey/analysis_scripts/csv_ouputs/wind_gust_{site}_stats.csv"
    )
...
def count_flyable_days(data_df, site):
    ...
    flyable_file = Path(f"csv_ouputs/wind_gust_{site}_fly_days.txt")
    ...
    ## otherwise
        ...
        with open(f"csv_ouputs/wind_gust_{site}_fly_days.txt", "wb") as file:
...
def is_day_flyable(day_df, days):
    ...
    for index, row in day_df.iterrows():
        ...
        ## otherwise
            ...
            gust_1 = row["Gust"]
            gust_2 = day_df.loc[day_df["Date and Time"] == consec_date_str, "Gust"]

            ## check if both precip values are below threshold
            if (gust_1 <= 10) and (gust_2.item() <= 10):
